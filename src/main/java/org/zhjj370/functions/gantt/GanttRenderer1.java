 ...
 */
public class GanttRenderer1 extends IntervalBarRenderer
     ...
     */
    public GanttRenderer1() {
        super();
        setIncludeBaseInRange(false);
        this.completePaint = Color.RED;
        this.incompletePaint = Color.GREEN;
    ...
    protected void drawTask(Graphics2D g2,
        ...
        PlotOrientation orientation = plot.getOrientation();
        RectangleEdge rangeAxisLocation = plot.getRangeAxisEdge();
        TaskSeriesLongCollection dataset = (TaskSeriesLongCollection) dataSet;
        // Y0
        //System.out.println("row="+row);
        List<Number> value0List = dataset.getEndValueList(row, column);
        if (value0List.size() == 0) {
            return;
        }
        // Y1
        List<Number> value1List = dataset.getStartValueList(row, column);
        if (value1List.size() == 0) {
            return;
        }

        for(int iValue=0;iValue<value0List.size();iValue++) {
            Number value0 = value0List.get(iValue);
            Number value1 = value1List.get(iValue);
            ...
            //todo
            double rectStart = domainAxis.getCategoryMiddle(column, getColumnCount(),
                    dataArea, plot.getDomainAxisEdge()) - state.getBarWidth()
                    / 2.0;

            double rectBreadth = state.getBarWidth()*((TaskSeriesLongCollection) dataSet).getSeriesCount();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof GanttRenderer1)) {
            return false;
        }
        GanttRenderer1 that = (GanttRenderer1) obj;
