...
/**
 * A collection of {@link TaskSeriesLong} objects.  This class provides one
 * implementation of the {@link GanttCategoryDataset} interface.
 */
public class TaskSeriesLongCollection extends AbstractSeriesDataset
     ...
     * Default constructor.
     */
    public TaskSeriesLongCollection() {
     ...
     * @since 1.0.1
     */
    public TaskSeriesLong getSeries(Comparable key) {
        if (key == null) {
            throw new NullPointerException("Null 'key' argument.");
        }
        TaskSeriesLong result = null;
     ...
     * @since 1.0.1
     */
    public TaskSeriesLong getSeries(int series) {
        if ((series < 0) || (series >= getSeriesCount())) {
            throw new IllegalArgumentException("Series index out of bounds");
        }
        return (TaskSeriesLong) this.data.get(series);
    }
    ...
    public Comparable getSeriesKey(int series) {
        TaskSeriesLong ts = (TaskSeriesLong) this.data.get(series);
        return ts.getKey();
    }
    ...
    public int getRowIndex(Comparable rowKey) {
        ...
        for (int i = 0; i < count; i++) {
            TaskSeriesLong s = (TaskSeriesLong) this.data.get(i);
    ...
    public Comparable getRowKey(int index) {
        TaskSeriesLong series = (TaskSeriesLong) this.data.get(index);
     ...
     */
    public void add(TaskSeriesLong series) {
        ...
        while (iterator.hasNext()) {
            TaskLong task = (TaskLong) iterator.next();
     ...
     */
    public void remove(TaskSeriesLong series) {
        Args.nullNotPermitted(series, "series");
    ...
    public void removeAll() {
        ...
        while (iterator.hasNext()) {
            TaskSeriesLong series = (TaskSeriesLong) iterator.next();
            series.removeChangeListener(this);
        }
    ...
    public Number getStartValue(Comparable rowKey, Comparable columnKey) {
        ...
        TaskSeriesLong series = (TaskSeriesLong) this.data.get(row);
        TaskLong task = series.get(columnKey.toString());
        if (task != null) {
            TimePeriodLong duration = task.getDuration();
            if (duration != null) {
     ...
     * @return The start value (possibly {@code null}).
     */
    public List<Number> getStartValueList(Comparable rowKey, Comparable columnKey) {
        List<Number> result = new ArrayList<Number>();
        int row = getRowIndex(rowKey);
        TaskSeriesLong series = (TaskSeriesLong) this.data.get(row);
        List<TaskLong> task = series.getTaskList(columnKey.toString());
        if (task.size()>0) {
            for(int i=0;i<task.size();i++) {
                TimePeriodLong duration = task.get(i).getDuration();
                if (duration != null) {
                    result.add(duration.getStart());
                }
            }
        }
        return result;
    ...
    public List<Number> getStartValueList(int row, int column) {
        Comparable rowKey = getRowKey(row);
        Comparable columnKey = getColumnKey(column);
        return getStartValueList(rowKey, columnKey);
    }
    ...
    public List<Number> getEndValueList(Comparable rowKey, Comparable columnKey) {

        List<Number> result = new ArrayList<>();
        int row = getRowIndex(rowKey);
        //System.out.println(row);
        TaskSeriesLong series = (TaskSeriesLong) this.data.get(row);
        //int x = getRowIndex(rowKey);
        //System.out.println(series.get(columnKey.toString()));
        List<TaskLong> task = series.getTaskList(columnKey.toString());
        if (task.size()>0) {
            for(int i=0;i<task.size();i++) {
                TimePeriodLong duration = task.get(i).getDuration();
                if (duration != null) {
                    result.add(duration.getEnd());
                }
            }
        }
        return result;
    ...
    public List<Number> getEndValueList(int row, int column) {
        Comparable rowKey = getRowKey(row);
        Comparable columnKey = getColumnKey(column);
        return getEndValueList(rowKey, columnKey);
    }
    ...
    public Number getStartValue(Comparable rowKey, Comparable columnKey,
        ...
        TaskLong task = series.get(columnKey.toString());
        if (task != null) {
            TaskLong sub = task.getSubtask(subinterval);
            if (sub != null) {
                TimePeriodLong duration = sub.getDuration();
                result = new Long(duration.getStart());
            }
        }
        return result;
    ...
    private void refreshKeys() {
        ...
        for (int i = 0; i < getSeriesCount(); i++) {
            TaskSeriesLong series = (TaskSeriesLong) this.data.get(i);
            // look for any keys that we don't already know about...
            Iterator iterator = series.getTasks().iterator();
            while (iterator.hasNext()) {
                TaskLong task = (TaskLong) iterator.next();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof TaskSeriesLongCollection)) {
            return false;
        }
        TaskSeriesLongCollection that = (TaskSeriesLongCollection) obj;
    ...
    public Object clone() throws CloneNotSupportedException {
        TaskSeriesLongCollection clone = (TaskSeriesLongCollection) super.clone();
